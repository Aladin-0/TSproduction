// src/components/JobSheetForm.tsx - Job Sheet Creation Form for Technicians
...
export const JobSheetForm: React.FC<JobSheetFormProps> = ({
  ...
  start_time: '09:00', // ADD DEFAULT TIME
  finish_time: '17:00', // ADD DEFAULT TIME
});
  ...
  const handleMaterialChange = (index: number, field: keyof JobSheetMaterial, value: any) => {
    setMaterials((prev) => {
      const updated = [...prev];
  ...
  const handleSubmit = async () => {
    // Validation
    if (!formData.equipment_type || !formData.problem_description || !formData.work_performed) {
      enqueueSnackbar('Please fill in all required fields', { variant: 'error' });
      return;
    }

    if (!formData.start_time || !formData.finish_time) {
      enqueueSnackbar('Please provide start and finish times', { variant: 'error' });
      return;
    }
    ...
    <StyledDialog open={open} onClose={handleClose} maxWidth="md" fullWidth>
      ...
      <DialogContent dividers sx={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12} sm={6}>
            <StyledTextField
              fullWidth
              label="Contact"
              value={serviceRequest.customer.phone}
              disabled
            />
          </Grid>
          <Grid item xs={12}>
            <StyledTextField
              ...
              value={`${serviceRequest.service_location.street_address}, ${serviceRequest.service_location.city}`}
              disabled
              multiline
              rows={2}
            />
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          ...
          <Grid item xs={12}>
            <StyledTextField
              ...
              multiline
              rows={4}
              placeholder="Describe the work you performed to fix the issue"
              required
            />
          </Grid>
        </Grid>
        ...
        {materials.length === 0 ? (
          ...
          >
            No materials added yet. Click "Add Material" to add materials used in this job.
          </Alert>
        ) : (
          materials.map((material, index) => (
            <MaterialRow key={index}>
              <Grid container spacing={2} alignItems="center">
                <Grid item xs={12} sm={3}>
                  <StyledTextField
                    ...
                    label="Date"
                    type="date"
                    value={material.date_used}
                    onChange={(e) =>
                      handleMaterialChange(index, 'date_used', e.target.value)
                    }
                    InputLabelProps={{ shrink: true }}
                    size="small"
                  />
                ...
                <Grid item xs={12} sm={4}>
                  <StyledTextField
                    ...
                    value={material.item_description}
                    onChange={(e) =>
                      handleMaterialChange(index, 'item_description', e.target.value)
                    }
                    size="small"
                  />
                </Grid>
                ...
                <Grid item xs={6} sm={2}>
                  <StyledTextField
                    ...
                    onChange={(e) =>
                      handleMaterialChange(index, 'unit_cost', parseFloat(e.target.value) || 0)
                    }
                    size="small"
                  />
                </Grid>
              ...
              <Typography
                variant="body2"
                sx={{ mt: 1, color: 'rgba(255, 255, 255, 0.7)', textAlign: 'right' }}
              >
        ...
        {materials.length > 0 && (
          <Box sx={{ textAlign: 'right', mt: 2 }}>
            <Typography variant="h6" sx={{ color: '#22c55e' }}>
              Total Material Cost: ₹
              {materials.reduce((sum, m) => sum + m.quantity * m.unit_cost, 0).toFixed(2)}
            </Typography>
          </Box>
        )}
      </DialogContent>
